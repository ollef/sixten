...
data Branches c d expr a
  ...
  = ConBranches [(c, Vector (NameHint, d), Scope Tele expr a)]
  | LitBranches [(Literal, expr a)] (expr a)
  deriving (Eq, Foldable, Functor, Ord, Show, Traversable)

instance Bound (Branches c d) where
  ConBranches cbrs   >>>= f = ConBranches [(c, t, s >>>= f) | (c, t, s) <- cbrs]
...
trimapBranches :: Bifunctor expr
              ...
              -> Branches c d' (expr x') y'
trimapBranches e f g (ConBranches cbrs) = ConBranches [(c, second e <$> t, bimapScope f g s) | (c, t, s) <- cbrs]
...
trifoldMapBranches :: (Bifoldable expr, Monoid m)
                  ...
                  -> Branches c d (expr x) y
                  -> m
trifoldMapBranches e f g (ConBranches cbrs) = fold [foldMap (foldMap e) t <> bifoldMapScope f g s | (_, t, s) <- cbrs]
...
tritraverseBranches e f g (ConBranches cbrs) =
  ConBranches <$> sequenceA [(,,) c <$> traverse (traverse e) t <*> bitraverseScope f g s | (c, t, s) <- cbrs]
...
exposeBranches f (ConBranches cbrs) =
  ConBranches [(c, t, exposeScope f s) | (c, t, s) <- cbrs]
