...
convertExpr :: Expr Meta -> ClosureConvert (Expr Meta)
...
knownCall f (tele, returnTypeScope) args
  | numArgs < arity = do
    vs <- lift $ forM (teleNames tele) $ \h -> forall h () Proxy
...
convertBranches
  :: Branches QConstr () Expr Meta
  -> ClosureConvert (Branches QConstr () Expr Meta)
